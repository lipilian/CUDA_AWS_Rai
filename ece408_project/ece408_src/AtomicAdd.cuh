...
// y4d(0,0,0,0) = a
  ...
  int totalZ = blockIdx.z;
  int currentLayer = totalZ/(C*K*K);
  int h = currentLayer / W_grid * TILE_WIDTH + threadIdx.y;
  int w = currentLayer % W_grid * TILE_WIDTH + threadIdx.x;
  int leftLayer = totalZ %(C*K*K);
  int c = leftLayer/(K * K);
  leftLayer = leftLayer % (K*K);
  int p = leftLayer / K;
  int q = leftLayer % K;
  if((h < H_out) && (w < W_out)){
    atomicAdd(&(y4d(b,m,h,w)), x4d(b, c, h + p, w + q) * k4d(m, c, p, q));
  }
  #undef y4d
  #undef x4d
  #undef k4d
